...
module.exports = class WebRTCAdaptor {
	...
	publish(
		...
		//If it started with playOnly mode and wants to publish now
			...
			this.mediaManager
				...
				.then(() => {
					var videoEnabled =
						this.mediaManager.localStream.getVideoTracks().length >
						0
							? true
							: false;
					var audioEnabled =
						this.mediaManager.localStream.getAudioTracks().length >
						0
							? true
							: false;
					this.sendPublishCommand(
	...
	iceCandidateReceived(event, streamId) {
		if (event.candidate) {
			...
			if (protocolSupported) {
				...
				if (this.debug) {
					console.log(
						'sending ice candiate for stream Id ' + streamId
					);
	...
	initPeerConnection(streamId, dataChannelMode) {
		if (this.remotePeerConnection[streamId] == null) {
			...
			if (this.dataChannelEnabled) {
				...
				if (dataChannelMode == 'publish') {
					...
					if (this.remotePeerConnection[streamId].createDataChannel) {
						...
						this.remotePeerConnection[streamId].ondatachannel =
							ev => {
								this.initDataChannel(streamId, ev.channel);
							};
					} else {
			...
			this.remotePeerConnection[streamId].oniceconnectionstatechange =
				event => {
					var obj = {
						state: this.remotePeerConnection[streamId]
							.iceConnectionState,
						streamId: streamId,
					};
					...
					if (
						!this.isPlayMode &&
						!this.playStreamId.includes(streamId)
					) {
						if (
							...
							this.mediaManager
								.changeBandwidth(
									this.mediaManager.bandwidth,
									streamId
								)
								.then(() => {
									console.log(
										'Bandwidth is changed to ' +
											this.mediaManager.bandwidth
									);
								})
	...
	closePeerConnection(streamId) {
		if (this.remotePeerConnection[streamId] != null) {
			...
			if (
				this.remotePeerConnection[streamId].signalingState != 'closed'
			) {
	...
	gotDescription(configuration, streamId) {
		this.remotePeerConnection[streamId]
			...
			.then(responose => {
				console.debug(
					'Set local description successfully for stream Id ' +
						streamId
				);
			...
			.catch(error => {
				console.error(
					'Cannot set local description. Error is: ' + error
				);
			});
	}
		...
		this.remotePeerConnection[streamId]
			...
			.then(response => {
				...
				if (isTypeOffer) {
					...
					console.log(
						'try to create answer for stream id: ' + streamId
					);
		...
		if (protocolSupported) {
			this.remotePeerConnection[streamId]
				...
				.then(response => {
					if (this.debug) {
						console.log(
							'Candidate is added for stream ' + streamId
						);
					}
				})
				.catch(error => {
	...
	getStats(streamId) {
		...
		this.remotePeerConnection[streamId].getStats(null).then(stats => {
			...
			this.remotePeerConnectionStats[streamId].audioPacketsLost =
				audioPacketsLost;
			this.remotePeerConnectionStats[streamId].fractionLost =
				fractionLost;
	...
	switchVideoCameraFacingMode(streamId, facingMode) {
		return this.mediaManager.switchVideoCameraFacingMode(
			streamId,
			facingMode
		);
	}
